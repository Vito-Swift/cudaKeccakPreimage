...
#define SIZEOF_U8 0x01UL
#define SIZEOF_U32 0x04UL
#define SIZEOF_U64 0x08UL
...
diff_eq(uint8_t *func, uint32_t idx, uint8_t *result) {
    memset(result, 0x0, (MQ_VAR_NUM + 1) * SIZEOF_U8);
...
fast_exhaustive(uint8_t *mqsystem, uint8_t *solution) {
    ...
    uint64_t term;
    for (var_idx = 0; var_idx < MQ_VAR_NUM; var_idx++) {
        memset(pdiff2[var_idx], 0x0, SIZEOF_U64 * MQ_VAR_NUM);
    ...
    memset(pdiff_eval, 0x0, SIZEOF_U64 * MQ_VAR_NUM);
    for (var_idx = 0; var_idx < MQ_VAR_NUM; var_idx++) {
        for (eq_idx = 0; eq_idx < MQ_EQ_NUM; eq_idx++) {
            if (var_idx == 0) {
    ...
    if (!func_eval) {
        ...
        memset(solution, 0x0, MQ_VAR_NUM * SIZEOF_U8);
    }
}
...
loadSystemsFromFile(KeccakSolver *keccakSolver) {
    ...
    for (i = 0; i < MQ_EQ_NUM; i++)
        mq_system[i] = (uint8_t *) malloc(BMQ_XVAR_NUM * SIZEOF_U8);
    uint8_t *append_system[AMQ_LIN_EQNUM];
    for (i = 0; i < AMQ_LIN_EQNUM; i++)
        append_system[i] = (uint8_t *) malloc(BMQ_XVAR_NUM * SIZEOF_U8);
...
keccakSolverInit(KeccakSolver *keccakSolver, int argc, char **argv) {
    ...
    for (uint32_t i = 0; i < 5; i++) {
        keccakSolver->test_message[i] = (uint32_t *) malloc(5 * SIZEOF_U32);
        memset(keccakSolver->test_message[i], 0, 5 * SIZEOF_U32);
    }
