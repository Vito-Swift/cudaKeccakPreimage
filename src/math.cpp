...
initMathSystem(MathSystem *system) {
    ...
    system->round3_lin_dep = (uint32_t *) malloc(800 * (IMQ_VAR_NUM + 1) * sizeof(uint32_t));

    // append system in order 2
    system->round3_append_system = (uint8_t *) malloc(AMQ_LIN_EQNUM * IMQ_XVAR_NUM * sizeof(uint8_t));

    // mq system in order 2
    system->round3_mq_system = (uint8_t *) malloc(AMQ_VAR_NUM * IMQ_XVAR_NUM * sizeof(uint8_t));
...
void
extractRound3LinearDependency(MathSystem *system, uint8_t **lin_system) {
    uint8_t tmprow[801];
    ...
    for (uint32_t i = 0; i < LIN_CONST_EQNUM; i++) {
        for (uint32_t j = 0; j < 800; j++) {
            if (lin_system[i][j]) {
                ...
                // lin_dep[j][j] = 0;
                memcpy(system->round3_lin_dep + (801 * j), lin_system[i], 801);
                *(system->round3_lin_dep + (801 * j) + j) = 0;
                break;
            }
        }
    }
...
reduceRound3AppendSystem(MathSystem *system, uint8_t **append_system) {
    ...
    uint32_t *mq2lin = system->round3_mq2lin;
                    ...
                    if (is_multix1_dep && is_multix2_dep) {
                        for (i = 0; i < IMQ_VAR_NUM; i++) {
                            for (j = 0; j < IMQ_VAR_NUM; j++) {
                                uint32_t tmp_idx1 = eqvar(multix_1, i, IMQ_VAR_NUM + 1);
                                uint32_t tmp_idx2 = eqvar(multix_2, j, IMQ_VAR_NUM + 1);
                                if (lin_dep[tmp_idx1] && lin_dep[tmp_idx2])
                                    reduced_system[eqvar(eq_idx, deg2midx2(i, j), IMQ_XVAR_NUM)] ^= 1;
                            }
                        }

                        if (lin_dep[eqvar(multix_1, IMQ_VAR_NUM, IMQ_VAR_NUM + 1)]) {
                            for (i = 0; i < IMQ_VAR_NUM; i++) {
                                if (lin_dep[multix_2 * (IMQ_VAR_NUM + 1) + i])
                                    reduced_system[eqvar(eq_idx, deg2midx1(IMQ_XVAR_NUM, i), IMQ_XVAR_NUM)] ^= 1;
                            }
                        }

                        if (lin_dep[eqvar(multix_2, IMQ_VAR_NUM, IMQ_VAR_NUM + 1)]) {
                            for (i = 0; i < IMQ_VAR_NUM; i++) {
                                if (lin_dep[multix_1 * (IMQ_VAR_NUM + 1) + i])
                                    reduced_system[eqvar(eq_idx, deg2midx1(IMQ_XVAR_NUM, i), IMQ_XVAR_NUM)] ^= 1;
                            }
                        }

                        if (lin_dep[eqvar(multix_1, IMQ_VAR_NUM, IMQ_VAR_NUM + 1)] &&
                            lin_dep[eqvar(multix_2, IMQ_VAR_NUM, IMQ_VAR_NUM + 1)])
                            reduced_system[eqvar(eq_idx, IMQ_XVAR_NUM - 1, IMQ_XVAR_NUM)] ^= 1;

                    } else if (is_multix1_dep) {
                        uint32_t tmp_idx = system->round3_lin2mq[multix_2];
                        for (i = 0; i < 800; i++) {
                            if (lin_dep[eqvar(multix_1, i, IMQ_VAR_NUM + 1)]) {
                                reduced_system[eqvar(eq_idx, deg2midx2(tmp_idx, i), IMQ_XVAR_NUM)] ^= 1;
                            }

                            if (lin_dep[eqvar(multix_1, IMQ_VAR_NUM, IMQ_VAR_NUM + 1)])
                                reduced_system[eqvar(eq_idx, deg2midx1(IMQ_VAR_NUM, tmp_idx), IMQ_XVAR_NUM)] ^= 1;
                        }

                    } else if (is_multix2_dep) {
                        uint32_t tmp_idx = system->round3_lin2mq[multix_1];
                        for (i = 0; i < 800; i++) {
                            if (lin_dep[eqvar(multix_2, i, IMQ_VAR_NUM + 1)]) {
                                reduced_system[eqvar(eq_idx, deg2midx2(tmp_idx, i), IMQ_XVAR_NUM)] ^= 1;
                            }
                        ...
                        reduced_system[eqvar(eq_idx, deg2midx2(tmp_idx1, tmp_idx2), IMQ_XVAR_NUM)] ^= 1;
                    }
                }
            }
        }
...
freeMathSystem(MathSystem *system) {
    free(system->round3_lin_dep);
    free(system->round3_mq2lin);
    free(system->round3_lin2mq);
    free(system->round3_append_system);
